...
-- ─────────────────────────────────────────────────────────────────────────────
-- 9. ADMIN ANALYTICS RPCs  (consumed by the admin panel; admin-only)
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  return (select jsonb_build_object(
    ...
    'active_drivers',       (select count(*) from public.drivers where approval_status = 'approved' and online_status <> 'offline'),
    ...
    'today_revenue',        coalesce((select sum(trip_price) from public.trips where status = 'completed' and completed_at >= date_trunc('day', now())), 0),
...
begin
  if not public.is_admin() then raise exception 'admin only' using errcode = 'insufficient_privilege'; end if;
  return query
    select n.title, n.type::text as target, to_char(max(n.created_at), 'YYYY-MM-DD') as sent,
           count(*)::int as reach,
           round(100.0 * count(*) filter (where n.is_read) / greatest(count(*), 1))::int as opened
    from public.notifications n group by n.title, n.type order by max(n.created_at) desc limit 50;
end; $$;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 10. BROADCAST DISPATCH + REALTIME + PAYMENTS
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Approved + online drivers can see open (unassigned) ride requests.
...
for select using (
  ...
  and exists (
    ...
    where d.profile_id = auth.uid() and d.approval_status = 'approved' and d.online_status = 'online'
  )
);
...
comment on table public.payments is 'One payment record per trip (fare, method and settlement status).';
